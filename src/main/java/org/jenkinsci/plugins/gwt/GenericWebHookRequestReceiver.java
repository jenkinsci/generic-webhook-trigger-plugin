...
public class GenericWebHookRequestReceiver extends CrumbExclusion implements UnprotectedRootAction {
    ...
    @VisibleForTesting
    int getGivenQuietPeriod(final Map<String, List<String>> headers, final Map<String, String[]> parameterMap) {
    ...
    String createMessageFromException(final Throwable t) {
        ...
        if (t.getStackTrace().length > 0) {
            stacktraceInfo =
                    "Thrown in: " + t.getStackTrace()[0].getClassName() + ":" + t.getStackTrace()[0].getLineNumber();
        }
        final String msg = "Exception occurred ("
                + t.getClass()
                + ": "
                + t.getMessage()
